...
def qwen3_lora(history, user_input, role):
    ...
    print(f"[{now_time()}] [qwen3-lora] Prompt:{history + history_tmp}")
    reply = requests.post(qwen3_lora_url, json={"history": history + history_tmp})
    reply = reply.json()
    if "<think>" in reply:
...
def ollama_qwen3_emotion(history: list):
    identity = f"你是一个情感分析助手，负责分析“丛雨”说的话的情感。你现在需要将用户输入的句子进行分析，综合用户的输入和丛雨的输出返回一个丛雨最新一句话每个分句情感的标签。你只可以选择的标签有{'，'.join(os.listdir(r'./reference_voices'))}。你需要直接返回一个情感列表，不需要其他任何内容。如[\"开心\", \"平静\"]/no_think"
    history_l = history[1:]
    prompt = {"model": "qwen3:14b",
...
def ollama_qwen25vl(image_path: str, source: str = "screen"):
    if source == "camera":
        identity = "你现在要担任一个AI桌宠的摄像头观察助手，请简要描述画面中用户的状态、动作和周围环境。我会将你的描述以system消息提供给另外一个处理语言的AI模型。不要猜测身份、隐私或无法确认的细节。"
    else:
        identity = "你现在要担任一个AI桌宠的视觉识别助手，我会向你提供用户此时的屏幕截图和历史记录，你要详细描述屏幕内容与使用的软件，描述页面主题。我会将你的描述以system消息提供给另外一个处理语言的AI模型。   "
    with open(image_path, "rb") as f:
        img_b64 = base64.b64encode(f.read()).decode()
    media_type = "image/jpeg" if source == "camera" else "image/png"
...
def resolve_reference_emotion(emotion: str) -> str:
    """将模型返回的情绪名映射到本地实际存在的参考音频目录。"""
    available = sorted(
        name
        for name in os.listdir("./reference_voices")
        if os.path.isdir(os.path.join("./reference_voices", name))
    )
    if not available:
        raise FileNotFoundError("reference_voices 下没有可用的情绪目录")

    requested = str(emotion).strip()
    if requested in available:
        return requested

    # 允许模型返回“被宠坏般的害羞”“有些生气”等带修饰词的情绪描述。
    for candidate in available:
        if candidate in requested or requested in candidate:
            print(
                f"[{now_time()}] [gpt-sovits-tts] 情绪 {requested!r} "
                f"映射为参考情绪 {candidate!r}"
            )
            return candidate

    # 无法匹配时使用平静，保证异常情绪不会让整个桌宠进程退出。
    fallback = "平静" if "平静" in available else available[0]
    print(
        f"[{now_time()}] [gpt-sovits-tts][WARN] 情绪 {requested!r} "
        f"不存在，回退到 {fallback!r}"
    )
    return fallback


def gpt_sovits_tts(sentence: str, emotion: str, aux_ref_audio_paths: list = []):
    """生成语音；单条语音失败时返回 None，不中断桌宠主流程。"""
    try:
        return _gpt_sovits_tts_impl(sentence, emotion, aux_ref_audio_paths)
    except Exception as exc:
        print(f"[{now_time()}] [gpt-sovits-tts][ERROR] 语音任务失败: {exc}")
        return None


def _gpt_sovits_tts_impl(sentence: str, emotion: str, aux_ref_audio_paths: list = []):
    emotion = resolve_reference_emotion(emotion)
