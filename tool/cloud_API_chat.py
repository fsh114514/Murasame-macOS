...
def cloud_emotion(history: list):
    identity = f"你是一个情感分析助手，负责分析“丛雨”说的话的情感。你现在需要将用户输入的句子进行分析，综合用户的输入和丛雨的输出返回一个丛雨最新一句话每个分句情感的标签。你只可以选择的标签有{'，'.join(os.listdir(r'./reference_voices'))}。你需要直接返回一个情感列表，不需要其他任何内容。如[\"开心\", \"平静\"]"
    history_l = history[1:]
    payload = {
        "messages": [{"role": "system", "content": identity},
    ...
    return reply
def cloud_vl(image_path: str, source: str = "screen"):
    API_key = get_config("./config.json")["APIKEY"]["qwen"]
    if source == "camera":
        identity = "你是一个AI桌宠的摄像头观察助手。请简要描述摄像头画面中用户的状态、动作和周围环境。我会将你的描述以system消息提供给另外一个处理语言的AI模型。只输出描述内容，不要猜测身份、隐私或无法确认的细节。"
    else:
        identity = "你是一个AI桌宠的屏幕观察助手。请简要描述用户正在做的事与使用的软件。我会将你的描述以system消息提供给另外一个处理语言的AI模型。只输出描述内容，且不要描述桌宠。"
    with open(image_path, "rb") as f:
        img_b64 = base64.b64encode(f.read()).decode()
    media_type = "image/jpeg" if source == "camera" else "image/png"

    payload = {
        "messages": [{"role": "user", "content": [{"type": "image_url","image_url": {"url": f"data:{media_type};base64,{img_b64}"}},
