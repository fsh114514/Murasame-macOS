...
class CapslockVoiceTrigger:
    ...
    def __init__(
        ...
        self.trigger_key_name = trigger_key.lower().strip()
        self.trigger_key = self._resolve_key(self.trigger_key_name)
    ...
    def _resolve_key(key_name: str):
        """将配置中的快捷键名称转换为 pynput 的按键对象。"""
        if key_name.startswith("f") and key_name[1:].isdigit():
            key = getattr(keyboard.Key, key_name, None)
            if key is not None:
                return key
        raise ValueError(f"不支持的语音触发键: {key_name}")
    ...
    def start(self) -> None:
        ...
        print(f"[AIpet][voice] {self.trigger_key_name.upper()} 语音触发已启动")
